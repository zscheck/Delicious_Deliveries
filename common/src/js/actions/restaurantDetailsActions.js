...
export const getMenuItems = restaurantId => {
  return (dispatch) => {
      Axios.get(`https://zs-delicious-deliveries.herokuapp.com/api/Restaurants/${restaurantId}/menus`)
      .then(res => res.data.id)
      .then(result => {
        console.log(':)', result);
        Axios.get(`https://zs-delicious-deliveries.herokuapp.com/api/Menus/${result}/menuItems`)
        .then(list => {
...
export function addOrder(delivery_address, order, custId) {
  ...
  return (dispatch) => {
      Axios.post(`https://zs-delicious-deliveries.herokuapp.com/api/Customers/${custId}/orders`, { delivery_address, order })
          .then(res => {
